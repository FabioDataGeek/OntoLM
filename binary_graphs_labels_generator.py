...
for file_path in tqdm(range(1000)):
    ...
    if f"text{file_path}.txt" in entries:
        ...
        with open(f"/usrvol/data/binary/labels/labels{iterator}.pickle", 'wb') as f:
            pkl.dump(labels, f)
            
        with open(f"/usrvol/data/binary/graphs/final_graphs{iterator}.pickle", 'wb') as f:
            pkl.dump(final_graphs, f)

with open('/usrvol/mappings/binary/NamesToInt.pickle', 'wb') as f:
...
with open("/usrvol/mappings/binary/vocab.txt", 'w') as f:
    for i in range(len(reverse_vocab_mapper)):
        value = IntToNames[reverse_vocab_mapper[i]]
        print(value, file=f)

with open('/usrvol/mappings/binary/vocab_mapper.pickle', 'wb') as f:
