...
def GNN_tensors(graph, graph_max_size):
    ...
    concepts_ids = concepts_ids.unsqueeze(0).unsqueeze(0)
    ...
    node_type = node_type.unsqueeze(0).unsqueeze(0)
    node_type = F.pad(node_type, p1d, "constant", 2)
    
    
    # Node_scores
    node_scores = torch.zeros(1,1,graph_max_size,1)
...
for file in files:
    with open(f"/usrvol/data/ISABIAL/graphs/{file}.pickle", 'rb') as f:
        graph_statements = pkl.load(f)
    graph_statements_tensors = [None] * 6
    for graph in tqdm(graph_statements):
        output = GNN_tensors(graph, 3500)
    ...
    with open(f"/usrvol/data/graph_statements_tensors/graph_statements{file}.pickle", 'wb') as f:
