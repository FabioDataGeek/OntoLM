...
EMB_PATHS = {
    ...
    'ddb': '/usrvol/experiments/data/binary/embs/ent_emb.npy',
}
...
def add_data_arguments(parser):
    ...
    # statements
    parser.add_argument('--train_statements', default='/usrvol/experiments/ISABIAL/binary/statements/train.statements.jsonl')
    parser.add_argument('--dev_statements', default='/usrvol/experiments/ISABIAL/binary/statements/dev.statements.jsonl')
    parser.add_argument('--test_statements', default='/usrvol/experiments/ISABIAL/binary/statements/test.statements.jsonl')
...
def add_optimization_arguments(parser):
    ...
    parser.add_argument('-bs', '--batch_size', default=80, type=int)
    ...
    parser.add_argument('--n_epochs', default=10, type=int, help='total number of training epochs to perform.')
...
def load_bert_xlnet_roberta_input_tensors(statement_jsonl_path, model_type, model_name, max_seq_length):
    class InputExample(object):
        ...
        def __init__(self, example_id, context, category, objective, label):
            ...
            self.category = category                        # "AminoAcidPeptideOrProtein"
            self.objective = objective                      # "hungtintin gene"
            self.label = label                              # False

    class InputFeatures(object):
        def __init__(self, example_id, choice_features, label):
            ...
            self.choice_features = [
                ...
                for _, input_ids, input_mask, segment_ids, output_mask in choice_features  # NOSOTROS SOLO EJECUTAMOS ESTO 1 VEZ PARA LA OPCIÓN BINARIA
            ]
    ...
    def convert_examples_to_features(examples, max_seq_length,
        ...
        for ex_index, example in enumerate(tqdm(examples)):
            ...
            tokens_c = tokenizer.tokenize(" " + classesToText[example.category.lower()])            
            ...
            features.append(InputFeatures(example_id=example.example_id, choice_features=choices_features, label=label))
    ...
    def select_field(features, field):
        return [[choice[field] for choice in feature.choice_features] for feature in features]
...
def load_sparse_adj_data_with_contextnode(adj_pk_path, max_node_num, num_choice):
    ...
    if use_cache:
        with open(cache_path, 'rb') as f:
            concept_ids, node_type_ids, node_scores, adj_lengths, edge_index, edge_type = pkl.load(f)
    # ESTE ELSE NO SE EJECUTA
    else:
        with open(adj_pk_path, 'rb') as fin:
            adj_concept_pairs = pkl.load(fin)
        ...
        with open(cache_path, 'wb') as f:
            pkl.dump([adj_lengths_ori, concept_ids, node_type_ids, node_scores, adj_lengths, edge_index, edge_type, half_n_rel], f)
    ...
    def forward(self, index, contextualized_emb=None):
        ...
        if contextualized_emb is not None:
            ...
            if hasattr(self, 'cpt_transform'):
                ...
                # se genera un valor de -1 en uno de los conceptos del índice...
...
class GATConvE(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_type, node_type, node_feature_extra, return_attention_weights=False):
        ...
        edge_vec = make_one_hot(edge_type, self.n_etype +1) #[E, 1141]
...
class QAGNN(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, sent_dim,
        ...
        self.concept_dim = concept_dim                          # 100 de base
    ...
    def forward(self, sent_vecs, concept_ids, node_type_ids, node_scores, adj_lengths, adj, emb_data=None, cache_output=False):
        ...
        # MODIFICADO, concept_ids[:, 1:]-1 parece ser que se resta 1 porque inicialmente empiezan a contar desde 1 y quieren que se cuente desde 0
        gnn_input1 = self.concept_emb(concept_ids[:, 1:], emb_data) #(batch_size, n_node-1, dim_node)
...
class LM_QAGNN(nn.Module):
    ...
    def forward(self, *inputs, layer_id=-1, args, cache_output=False, detail=False):
        ...
        # poner tamaño de batch size
        #inputs[7] = torch.reshape(inputs[7], (args.batch_size,1))
        inputs[7] = torch.unsqueeze(inputs[7], 0)
        ...
        """ # Separamos las entradas en tres partes
        inputs_1 = inputs[:-6]
        inputs_2 = inputs[-6:-2]
        inputs_3 = inputs[-2:] """

        # Reformulamos las primeras dos partes
        #inputs_1_2 = [x.view(x.size(0) * x.size(1), *x.size()[2:]) for x in inputs_1 + inputs_2]

        # Combinamos las listas de entradas
        """ combined_inputs = inputs_1 + inputs_2 """

        # Creamos una lista vacía para almacenar los resultados
        """ inputs_1_2 = [] """

        """  # Iteramos sobre las entradas combinadas
        for x in combined_inputs:
            # Reformulamos cada entrada y la añadimos a la lista de resultados
            reshaped_x = x.view(x.size(0) * x.size(1), *x.size()[2:])
            inputs_1_2.append(reshaped_x)


        # Reformulamos la tercera parte
        inputs_3 = [sum(x, []) for x in inputs_3]

        # Juntamos todas las partes
        _inputs = inputs_1_2 + inputs_3 """
...
class LM_QAGNN_DataLoader(object):
    ...
    def __init__(self, args, train_statement_path, train_adj_path,
                 ...
                 batch_size, eval_batch_size, device, model_name, max_node_num=3500, max_seq_length=512,
        ...
        *self.train_decoder_data, self.train_adj_data = load_sparse_adj_data_with_contextnode(train_adj_path, max_node_num, num_choice)
        *self.dev_decoder_data, self.dev_adj_data = load_sparse_adj_data_with_contextnode(dev_adj_path, max_node_num, num_choice)
        ...
        if test_statement_path is not None:
            ...
            *self.test_decoder_data, self.test_adj_data = load_sparse_adj_data_with_contextnode(test_adj_path, max_node_num, num_choice)
...
def train(args):
    ...
    # try:
        for epoch_id in range(args.n_epochs):
            ...
            metrics.append({'epoch': epoch_id, 'step': global_step, 'dev_acc': dev_acc, 'test_acc': test_acc})
...
def main():
    ...
    args.train_adj = '/usrvol/data/binary/graph_statements_tensors/graph_statementstrain.pickle' 
    args.dev_adj = '/usrvol/data/binary/graph_statements_tensors/graph_statementsdev.pickle'
    args.test_adj = '/usrvol/data/binary/graph_statements_tensors/graph_statementstest.pickle'
    args.train_statements = '/usrvol/data/binary/ISABIAL/statements/train.statements.jsonl'
    args.dev_statements = '/usrvol/data/ISABIAL/binary/statements/dev.statements.jsonl'
    args.test_statements = '/usrvol/data/ISABIAL/binary/statements/test.statements.jsonl'
    args.encoder = 'bert'
    args.ent_emb = ent_emb
    args.ent_emb_paths = ['/usrvol/experiments/data/binary/embs/ent_emb.npy']
    #args.save_model
    ...
    parser.add_argument('--train_adj', default='/usrvol/experiments/data/binary/graph_statements_tensors/graph_statementstrain.pickle')
    parser.add_argument('--dev_adj', default='/usrvol/experiments/data/binary/graph_statements_tensors/graph_statementsdev.pickle')
    parser.add_argument('--test_adj', default='/usrvol/experiments/data/binary/graph_statements_tensors/graph_statementstest.pickle')
    ...
    parser.add_argument('--gnn_dim', default=300, type=int, help='dimension of the GNN layers')
    ...
    parser.add_argument('-mbs', '--mini_batch_size', default=80, type=int) # 1
